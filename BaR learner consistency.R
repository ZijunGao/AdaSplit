...
setting = 2
# n = 300 # sample size
n.seq = c(200, 400, 600, 800, 1000) # c(200, 400, 800, 1600) # sequence of sample sizes
...
record = list()
record$betaError = record$R2 = matrix(0, nrow = n_trial, ncol = length(n.seq))
...
for(n.index in seq(1, length(n.seq))){
  ...
  for (j in 1:n_trial){
    ...
    tau.hat.active = nuisance.tau.active(Y = Y, X = X, Ex = Ex, W = W, mu = mu.hat, Group = Group, proportion = proportion, lambda.update.period = 40, loss_threshold = -Inf) #estimate tau using active learning
    ...
    record$betaError[j, n.index] = sum((tau.hat.active$beta - beta)^2)
...
apply(record$R2, 2, mean)
apply(record$betaError, 2, mean)
