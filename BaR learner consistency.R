...
setting = 2
# n = 300 # sample size
n.seq = 300 * seq(1, 5) # 300 * seq(1, 5) # c(200, 400, 800, 1600) # sequence of sample sizes
Group.number = 5 # total number of groups
delta = 0.5 # effect size
sigma = 0.2 # std of noise
n_trial = 2 # number of runs
proportion = 0.5 # 0.5; proportion of randomness in the nuisance fold
num_features = 5
...
for(n.index in seq(1, length(n.seq))){
  ...
  for (j in 1:n_trial){
    ...
    # BaR-learner
    tau.hat.active = nuisance.tau.active(Y = Y, X = X, Ex = Ex, W = W, mu = mu.hat, Group = Group, proportion = proportion, lambda = 1, lambda.update.period = 50, loss_threshold = -Inf, marginalize = T, initial.proportion = 0.2) #estimate tau using active learning
