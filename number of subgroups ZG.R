...
for(j in 1 : length(Group.level.number2.seq)){
  ...
  for(i in 1:m){
    ...
    tau = delta * (S[, 1] >= Group.level.number[1]) * (S[, 2] >= (Group.level.number[2] * (3 / 4) + 1)) * ((setting != "HTE") + (setting == "HTE") * X[, 1]) #  (S[, 1] >= 3) * (S[, 2] >= 3)
    ...
    record$pValue$SSRT[i,seq(1, Group.number),j] = SS(Y = Y, X = X, G = G, Group = Group, prop = p, M = M, test.stats.method = test.stats.method)$pval
    ...
    record$pValue$DDRT[i,seq(1, Group.number),j] = DD(Y = Y, X = X, G = G, Group = Group, prop = p, M = M, test.stats.method = test.stats.method)$pval
    ...
    # ART: augmented RT
    record$pValue$ART[i,seq(1, Group.number),j] = ART(Y = Y, X = X, G = G, Group = Group, prop = p, M = M, test.stats.method = test.stats.method, B = B)$pval
