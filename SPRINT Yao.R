...
Y = data$T_PRIMARY*(1-data$EVENT_PRIMARY) #+ data$T_PRIMARY 
  #(data$T_PRIMARY>800)*data$T_PRIMARY #*(1-data$EVENT_PRIMARY)
...
#########################
feature_list = c('10-year CVD risk score','Urine Albumin/Creatinine ratio')
...
matrix1 <- matrix(record$pValue$RT, nrow = 5, byrow = TRUE)
matrix2 <- matrix(record$pValue$ART, nrow = 5, byrow = TRUE)
...
lighter_colors <- colorRampPalette(c("lightcoral", "white", "lightblue"))(200)
