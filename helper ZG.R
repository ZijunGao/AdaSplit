...
permutation.p.value = function(stats, stats.ref, type = "two tailed"){
  ...
  if(type == "right tailed"){
    ...
    # right.tailed.p.value = (sum(stats <= stats.ref[!is.na(stats.ref)]) + 1) / (length(stats.ref[!is.na(stats.ref)]) + 1)
...
test.stats = function(Y, W, X = NULL, G = NULL, stats = "denoise", prop = NULL, mu0.hat = NULL, mu1.hat = NULL, mu.hat = NULL, tau.hat = NULL) {
  ...
  if (stats == "plain") {
    ...
    # value = abs(sum(W * (Y - mu.hat)) / n1 - sum((1 - W) * (Y - mu.hat)) / n0) # two-sided
    value = sum(W * (Y - mu.hat)) / n1 - sum((1 - W) * (Y - mu.hat)) / n0 # one-sided
  }else if(stats == "denoise normalized"){
    IF = W * (Y - mu.hat) / prop - (1 - W) * (Y - mu.hat) / (1-prop)
    # value = abs(mean(IF)) / sd(IF) # two-sided
    value = mean(IF) / sd(IF) # one-sided
  }else if (stats == "ATE") {
    ...
    # value = abs(mean(IF)) / sd(IF) # two-sided
    value = mean(IF) / sd(IF) # one-sided
    # var.estimate = var((Y - mu1.hat)[W == 1])/prop + var((Y - mu0.hat)[W == 0])/(1 - prop) # Neymanian resampled variance
  }else if (stats == "ITE") {
...
ORT = function(Y, W, X, G, Group, prop = NULL, mu0, mu1, mu, tau, test.stats.method = "denoise",  treatment.assignment = "Bernoulli", M = 10) {
  ...
  test.stats.ref = t(matrix(replicate(M, test.stats.group(Y = Y, W = rbinom(n, 1, prop), X = X, G = G, Group = Group, stats = test.stats.method, prop = prop, mu0.hat = mu0, mu1.hat = mu1, mu.hat = mu, tau.hat = tau)), ncol = M)) #   Each column represents a group
