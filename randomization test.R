...
record = list()
record$pValue = list()
record$pValue$ORT$fusion = record$pValue$ORT$denoise = record$pValue$ORT$denoise.normalized = record$pValue$ORT$AIPW = record$pValue$ORT$AIPW.normalized = matrix(0, nrow = m, ncol = Group.number)
...
for(i in 1:m){
  ...
  tau = delta * (S[, 1] >= (Group.level.number[1]-1)) * (S[, 2] >= (Group.level.number[2] - 1)) + 2 * X[,2] # * (1 + X[, 2]^2) #  (1+ X[, 2]^2)
  ...
  record$pValue$ORT$fusion[i,] = ORT(Y = Y, W = W, X = X, G = G, Group = Group, prop = p, test.stats.method = "fusion", mu0 = mu0.hat, mu1 = mu1.hat, mu = mu.hat, tau = tau.hat, M = M)$pval
...
for(i in 1){
  ...
  matplot(cdf.curve, main = names(record$pValue)[i], x = seq(1, 100)/100, xlab = "alpha", type = "l", lty = 1, col = seq(2, 6), ylim = c(0, 1)); legend("bottomright", colnames(cdf.curve), col = seq(2, 6), lty = 1); abline(a = 0, b = 1, lty = 3)
}
