...
record = list()
record$pValue = list()
record$pValue$ORT = record$pValue$RT = record$pValue$SSRT = record$pValue$DDRT = record$pValue$ART = array(0, dim = c(m, Group.number, length(delta.seq))) # ORT: oracle RT; RT (baseline): standard RT; SSRT: sample-splitting RT; DDRT: double-dipping RT; ART: augmented RT; 
record$R = list(); record$R$ART = list(); for(j in seq(1, length(delta.seq))){record$R$ART[[j]] = list()}; record$R$ORT = record$R$RT = record$R$SSRT = record$R$DDRT = record$R$ART
record$FDP = list();record$FDP$ORT = record$FDP$RT = record$FDP$SSRT = record$FDP$DDRT = record$FDP$ART = matrix(0, m, length(delta.seq)) 
record$power = record$FDP

set.seed(318)
for(j in 1 : length(delta.seq)){
  delta = delta.seq[j]
  for(i in 1:m){
    ...
    record$pValue$ORT[i,,j] = ORT(Y = Y, W = W,  X = X, G = G, Group = Group, prop = p, test.stats.method = test.stats.method, mu0 = mu0, mu1 = mu1, mu = mu, tau = tau, M = M)$pval
    ...
    record$pValue$RT[i,,j] = RT(Y = Y, W = W,  X = X, G = G, Group = Group, prop = p, M = M)$pval
    ...
    record$pValue$SSRT[i,,j] = SS(Y = Y, W = W,  X = X, G = G, Group = Group, prop = p, M = M, nuisance.learner.method = nuisance.learner.method, test.stats.method = test.stats.method)$pval
    ...
    record$pValue$DDRT[i,,j] = DD(Y = Y, W = W,  X = X, G = G, Group = Group, prop = p, M = M, nuisance.learner.method = nuisance.learner.method, test.stats.method = test.stats.method)$pval
...
# results
result = list()
result$FDR = data.frame(lapply(record$FDP, function(x){apply(x, 2, mean)})); rownames(result$FDR) = delta.seq
result$power = data.frame(lapply(record$power, function(x){apply(x, 2, mean)})); rownames(result$power) = delta.seq
result
