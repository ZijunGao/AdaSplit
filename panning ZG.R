...
for(i in 1:m){
  ...
  record$pValue$RT[i,] = sapply(seq(1, Group.number), function(x){return(mean(abs(T.RT.ref[, x]) >= abs(T.RT[x]), na.rm = T))})
  ...
  record$pValue$SSRT[i,] = sapply(seq(1, Group.number), function(x){return(mean(abs(T.SSRT.ref[, x]) >= abs(T.SSRT[x]), na.rm = T))})
  ...
  record$pValue$ART[i,] = sapply(seq(1, Group.number), function(x){return(mean(abs(T.ART.ref[, x]) >= abs(T.ART[x]), na.rm = T))})
