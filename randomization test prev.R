...
# setting
setting = "HTE3" # "default", "HTE", "HTE2", "HTE3", "HTE4", "misspecified"
...
for(i in 1:m){
  ...
  # mu.hat = mu # mu
  # tau.hat = tau 
  # negatively correlated mu.hat, tau.hat
  mu.hat = mu + rnorm(n, 0, 4) # mu
  tau.hat = tau - (mu.hat - mu) # tau - (mu.hat - mu) / (1 - 2 * p) 
...
# denoise
# mu.hat = mu
test = ORT(Y = Y, W = W, X = X, G = G, Group = Group, prop = p, test.stats.method = "denoise", mu0 = mu0.hat, mu1 = mu1.hat, mu = mu.hat, tau = tau.hat, M = M)

W.knockoff = matrix(rbinom(n * B, 1, p), ncol = B) 
first.moment.hat = sum(apply(cbind(W, W.knockoff), 2, function(x){mean(x * ((Y - mu.hat) / p) + (1-x) * (-(Y - mu.hat) / (1-p)))})) # mean(mu1.hat - mu0.hat)
(result.denoise = data.frame(empirical.ref.mean = mean(test$test.stats.ref),
                             ...
                             estimated.test.stats = mean(first.moment.hat)))

# AIPW normalized
test = ORT(Y = Y, W = W, X = X, G = G, Group = Group, prop = p, test.stats.method = "AIPW normalized", mu0 = mu0.hat, mu1 = mu1.hat, mu = mu.hat, tau = tau.hat, M = M)
...
(result.AIPW.normalized = data.frame(empirical.ref.mean = mean(test$test.stats.ref),
                                     ...
                                     estimated.test.stats =  mean(first.moment.hat) / sqrt(max(1e-6, second.moment.hat - first.moment.hat^2))))
